...
summarize_matrix <- function(x, na.rm=FALSE) {
    result <- data.frame(mean = apply(x, 1, mean), 
                         stdev = apply(x, 1, sd),
                         median = apply(x, 1, median),
                         min = apply(x, 1, min),
                         max = apply(x, 1, max),
                         num_lt_0 = apply(x, 1, function(x) sum(less_than_zero(x))),
                         num_btw_1_and_5 = apply(x, 1, function(x) sum(is_between(x, 1, 5))),
                         num_na = apply(x, 1, function(x) sum(is.na(x)))
                         )
    return(result)
}
