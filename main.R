...
sum_na <- function(x, na.rm = FALSE){
  if(na.rm){
    return(sum(is.na(!is.na(x))))
  }else{
    return(sum(is.na(x)))
  }
}
...
summarize_matrix <- function(x, na.rm=FALSE) {
    result <- data.frame(mean = apply(x, 1, mean, na.rm = na.rm),
                         ...
                         num_lt_0 = apply(x, 1, function(x) sum(less_than_zero(x), na.rm = na.rm)),
                         num_btw_1_and_5 = apply(x, 1, function(x) sum(is_between(x, 1, 5), na.rm = na.rm)),
                         num_na = apply(x, 1, function(x) sum_na(x, na.rm = na.rm))
     )
